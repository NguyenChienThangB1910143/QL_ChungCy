        ...
        <div class="mb-3 option-baixe">
            ...
            @foreach($baixe as $baixe)
                @if($baixe->loaixe == 'Xe máy')
                    <option value="{{$baixe->id_baixe}}" data-id-loaixe="{{$baixe ->loaixe}}">{{$baixe->ms}}</option>
                @endif
            @endforeach
            </select>
        </div>
...
<script>
    ...
    document.getElementById('loaixe').addEventListener('change', function() {
        ...
        loaixeOptions.forEach(function(option) {
            if (option.getAttribute('data-id-laixe') == selectedLoaixe) {
                option.style.display = 'block';
            } else {
                option.style.display = 'none';
            }
