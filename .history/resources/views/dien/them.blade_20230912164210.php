...
<script>
    ...
    document.getElementById('phong').addEventListener('change', function() {
    var selectedPhong = this.value;

    // Gửi yêu cầu AJAX tới server để lấy chỉ số mới trước đó
    $.get('/dien/them/chiso_moi/' + selectedPhong, function(data) {
        // Điền chỉ số mới trước đó vào trường chỉ số cũ
        document.querySelector('input[name="chiso_cu"]').value = data.chiso_moi;
    });
});
